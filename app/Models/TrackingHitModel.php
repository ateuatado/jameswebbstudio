...
class TrackingHitModel extends Model
{
    ...
    public function hitsBySource(string $from, string $to): array
    {
        return $this->db->table('tracking_hits th')
            ...
            ->groupBy('tl.utm_source')
            ->orderBy('total', 'DESC')
            ->get()->getResultObject();
    }

    public function hitsByDay(string $from, string $to, ?int $linkId = null): array
    {
    ...
    public function hitsByCampaign(string $from, string $to): array
    {
        return $this->db->table('tracking_hits th')
            ...
            ->where('th.created_at >=', $from)
            ->where('th.created_at <=', $to)
            ->groupBy('tl.utm_campaign')
            ->orderBy('total', 'DESC')
            ->get()->getResultObject();
    }

    public function hitsByCity(string $from, string $to, int $limit = 10): array
    {
        return $this->db->table('tracking_hits')
    ...
    public function hitsByDevice(string $from, string $to): array
    {
        return $this->db->table('tracking_hits')
